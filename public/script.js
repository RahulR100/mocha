...
var myPeer = new Peer(undefined, {
	...
	secure: true,
	config: {
		iceServers: [{ url: 'stun:mochaturn.xyz' }, { url: 'turn:mochaturn.xyz', username: 'mocha', credential: 'mochavideo' }]
	}
});
...
socket.on('create-message', (message, userName) => {
	...
	messages.innerHTML += `<div class="message ${appendClass}">
		<b><span> ${userName === myUsername ? 'me' : userName}</span></b>
		<span>${message}</span>
	</div>`;
});
...
socket.on('add-agenda', (agendaItem, agendaRaw, userName, uuid) => {
	agenda.innerHTML += `<div class="agenda" id="${uuid}" raw='${agendaRaw}'>
		<b><span> ${userName === myUsername ? 'me' : userName}</span></b>
